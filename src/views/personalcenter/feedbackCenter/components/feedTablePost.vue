<template>
  <h2 class="page-title">Post-UsabilitySurvey Table</h2>
  <div class="feedTablePost">
    ...
    <div class="demo-pagination-block pagination" style="margin-top: 30px">
      <el-pagination
        v-model:current-page="pageTable1.pageNum"
        v-model:page-size="pageTable1.pageSize"
        :page-sizes="[100, 200, 300, 400]"
        :size="size"
        :disabled="disabled"
        :background="background"
        layout="total, sizes, prev, pager, next, jumper"
        :total="props.table1.total"
...
<script setup lang="ts">
  ...
  const tableData = computed(() =>
    props.table1.list.map(item => {
      const survey = item['Post-UsabilitySurvey'];
      const array = survey
  ...
  const emit = defineEmits<{
    (
      e: 'pageChange',
      params: { pageNum: number; pageSize: number; name: 'table1' },
    ): void;
    (
      e: 'pageSizeChange',
      params: { pageNum: number; pageSize: number; name: 'table1' },
  ...
  const handleSizeChange = (val: number) => {
    emit('pageSizeChange', {
      pageNum: pageTable1.value.pageNum,
      pageSize: val,
      name: 'table1',
    });
  };
  const handleCurrentChange = (val: number) => {
    emit('pageChange', {
      pageNum: val,
      pageSize: pageTable1.value.pageSize,
      name: 'table1',
    });
  };
</script>
